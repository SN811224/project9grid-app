...
class Repo {
  ...
  Future<List<Map<String, dynamic>>> recruitmentLogs(String recruitmentId) async {
    final rows = await client
  ...
  Future<Map<String, dynamic>?> prospectBySourceContact(String contactId) async {
    final rows = await client
        .from('prospects')
        .select()
        .eq('user_id', uid)
  ...
  Future<void> replacePolicyRiders(
    ...
    await client.from('policy_riders').insert(
      riders.map((e) => {'user_id': uid, 'policy_id': policyId, ...e}).toList(),
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    final filtered = rows.where((row) {
      ...
      ..sort((a, b) {
        ...
        final da = lastCustomerProgress(a) ??
            DateTime.fromMillisecondsSinceEpoch(0);
        final db = lastCustomerProgress(b) ??
            DateTime.fromMillisecondsSinceEpoch(0);
...
class _CustomerDetailPageState extends State<CustomerDetailPage> {
  ...
  Future<void> addFollowLog() async {
    ...
    try {
      await repo.insert('follow_logs', {
        ...
        'contacted_at': DateTime(date.year, date.month, date.day).toIso8601String(),
      });
      await load();
  ...
  Future<void> editFollowLog(Map<String, dynamic> row) async {
    ...
    String channel = textOf(row['channel']).isEmpty ? '電話' : textOf(row['channel']);
  ...
  Future<void> edit([Map<String, dynamic>? row]) async {
    ...
    try {
      ...
      final alreadyConverted =
          savedContact?['converted_customer_id'] != null;
      if ((status ?? '未聯絡') == '已成交' &&
          !alreadyConverted &&
          savedContact != null) {
                      ...
                      trailing: PopupMenuButton<String>(
                        onSelected: (value) async {
                          if (value == 'edit') {
                            ...
                            await confirmDelete(context, () async {
                              await repo.remove('contacts', row['id'].toString());
                              await load();
                            });
                          }
                        },
...
class _ProspectsPageState extends State<ProspectsPage>
  ...
  Future<void> edit([Map<String,dynamic>? row]) async {
        ...
        child:SingleChildScrollView(child:Column(children:[
          if(row==null)
            addModalHeader(
              ...
              '新增經營名單',
              ()=>[
                name.text,phone.text,lineId.text,birthday.text,
                occupation.text,company.text,family.text,notes.text
              ].any((e)=>e.trim().isNotEmpty),
            )
          else
            const Text('編輯經營名單',
              style:TextStyle(fontSize:24,fontWeight:FontWeight.w900)),
          const SizedBox(height:18),
          field(name,'姓名 *'),field(phone,'手機'),field(lineId,'LINE ID'),
          field(birthday,'生日（民國YYY-MM-DD）'),field(occupation,'職業'),
          field(company,'公司'),field(family,'家庭狀況'),
          DropdownButtonFormField<String>(
    ...
    final values={
      'name':name.text.trim(),'phone':blank(phone.text),'line_id':blank(lineId.text),
      'birthday':normalizedBirthday,'occupation':blank(occupation.text),
      'company':blank(company.text),'family_status':blank(family.text),
      'source_category':sourceType,
      'referred_by_name':sourceType=='自行新增'
          ? '自行新增'
          : blank(referredBy.text),
      'notes':blank(notes.text),
      'status':row?['status']?.toString()=='已聯絡'
          ? '經營中'
          : (row?['status']?.toString()??'經營中'),
    };
    if(row==null){await repo.insert('prospects',values);}
    else{await repo.update('prospects',row['id'].toString(),values);}
    await load();
  }
  ...
  @override Widget build(BuildContext context){
    ...
    final active=rows.where((e)=>e['status']!='已轉客戶').where((e){
      final h=[e['name'],e['phone'],e['occupation'],e['company'],e['notes'],e['referred_by_name']]
        .whereType<Object>().join(' ').toLowerCase();
    ...
    return Scaffold(
      ...
      floatingActionButton:FloatingActionButton.extended(
        onPressed:()=>edit(),icon:const Icon(Icons.person_add_alt_1),label:const Text('新增經營')),
      body:Column(children:[
        Container(
          padding:const EdgeInsets.fromLTRB(20,18,20,20),
          decoration:const BoxDecoration(color:navy,borderRadius:BorderRadius.vertical(bottom:Radius.circular(34))),
          child:TextField(controller:search,onChanged:(_)=>setState((){}),
            decoration:InputDecoration(hintText:'搜尋姓名、手機、職業、公司或備註',
              prefixIcon:const Icon(Icons.search),filled:true,fillColor:Colors.white,
              border:OutlineInputBorder(borderRadius:BorderRadius.circular(18),borderSide:BorderSide.none)))),
        Expanded(child:active.isEmpty?const Center(child:Text('目前沒有經營名單')):
          RefreshIndicator(onRefresh:load,child:ListView.separated(
            padding:const EdgeInsets.fromLTRB(16,18,16,100),itemCount:active.length,
            separatorBuilder:(_,__)=>const SizedBox(height:10),
            itemBuilder:(context,index){
              final row=active[index];
              final referredByName = textOf(row['referred_by_name']).isNotEmpty
              ...
              return Card(child:ListTile(
                onTap:() async {
                  await Navigator.push(context,MaterialPageRoute(builder:(_)=>ProspectDetailPage(prospect:row)));
                  await load();
                },
                title:Row(children:[
                  followUpDot(
                ...
                trailing:PopupMenuButton<String>(
                  onSelected:(v) async {
                    if(v=='edit'){await edit(row);}
                    else{await confirmDelete(context,() async{await repo.remove('prospects',row['id'].toString());await load();});}
                  },
                  itemBuilder:(_)=>const[
...
class ProspectDetailPage extends StatefulWidget {
  ...
  @override State<ProspectDetailPage> createState()=>_ProspectDetailPageState();
}

class _ProspectDetailPageState extends State<ProspectDetailPage>{
  final repo=Repo(); final search=TextEditingController();
  List<Map<String,dynamic>> logs=[];
  ...
  Future<void> addLog() async{
    final when=TextEditingController(text:DateFormat('yyyy-MM-dd').format(DateTime.now()));
    ...
    final saved=await showModalBottomSheet<bool>(
      ...
      builder:(context)=>Padding(
        ...
        child:SingleChildScrollView(child:Column(children:[
          ...
          const SizedBox(height:18),field(when,'經營日期（YYYY-MM-DD）'),
          field(status,'目前經營狀況'),field(content,'經營備註 / 內容 *',maxLines:5),
          FilledButton(onPressed:()=>Navigator.pop(context,true),child:const Text('儲存本次紀錄')),
        ])),
      ),
    );
    ...
    try{
      await repo.insert('engagement_logs',{
        ...
        'engaged_at':DateTime(date.year,date.month,date.day).toIso8601String(),
  ...
  Future<void> convert() async{
    ...
    final ok=await showDialog<bool>(context:context,builder:(context)=>AlertDialog(
      title:const Text('轉為成交客戶'),content:Text('將「${textOf(p['name'])}」轉入客戶名單？\n原推薦關係與經營紀錄都會保留。'),
      actions:[
        TextButton(onPressed:()=>Navigator.pop(context,false),child:const Text('取消')),
        FilledButton(onPressed:()=>Navigator.pop(context,true),child:const Text('轉入客戶')),
      ]));
    if(ok!=true)return;
    final c=await repo.insertReturning('customers',{
      'name':textOf(p['name']),'phone':p['phone'],'line_id':p['line_id'],
      'birthday':p['birthday'],'occupation':p['occupation'],'company':p['company'],
      'family_status':p['family_status'],'notes':p['notes'],
      'closed_date':DateTime.now().toIso8601String().split('T').first,
    ...
    if(mounted){snack(context,'已加入成交客戶，推薦鏈已保留');Navigator.pop(context);}
  }

  @override Widget build(BuildContext context){
    final p=widget.prospect; final k=search.text.trim().toLowerCase();
    final filtered=logs.where((e)=>k.isEmpty||
      [e['content'],e['status_note'],e['engaged_on']].whereType<Object>().join(' ').toLowerCase().contains(k)).toList();
    return Scaffold(
      appBar:AppBar(title:Text(textOf(p['name']))),
      bottomNavigationBar: mainNavigationBar(context, 3),
      floatingActionButton:FloatingActionButton.extended(onPressed:addLog,
        icon:const Icon(Icons.add_comment_outlined),label:const Text('新增經營紀錄')),
      body:ListView(padding:const EdgeInsets.fromLTRB(16,8,16,110),children:[
        Card(child:Padding(padding:const EdgeInsets.all(18),child:Column(children:[
          infoRow('手機',textOf(p['phone'])),infoRow('LINE',textOf(p['line_id'])),
          infoRow('生日',formatRocBirthday(p['birthday'])),infoRow('職業',textOf(p['occupation'])),
          infoRow('公司',textOf(p['company'])),infoRow('家庭',textOf(p['family_status'])),
          infoRow(
            '來源',
        ...
        FilledButton.icon(onPressed:convert,icon:const Icon(Icons.person_add_alt_1),label:const Text('轉為成交客戶')),
        const SizedBox(height:14),
        TextField(controller:search,onChanged:(_)=>setState((){}),
          decoration:const InputDecoration(labelText:'搜尋經營紀錄',prefixIcon:Icon(Icons.search),border:OutlineInputBorder())),
        sectionTitle('經營歷程 ${logs.length} 筆'),
        if(filtered.isEmpty)const Card(child:Padding(padding:EdgeInsets.all(18),child:Text('尚無符合的經營紀錄'))),
        ...filtered.map((e)=>Card(child:ListTile(
          onTap:()=>editLog(e),
          leading:const Icon(Icons.history),
          title:Text(textOf(e['content'])),
          subtitle:Text([
          ...
          trailing:PopupMenuButton<String>(
            onSelected:(value) async{
              if(value=='edit'){
                ...
                await confirmDelete(context,() async{
                  await repo.remove('engagement_logs',e['id'].toString());
                  await load();
                });
              }
            },
...
class _RecruitmentPageState extends State<RecruitmentPage>
  ...
  Future<void> edit([Map<String,dynamic>? row]) async {
    ...
    String sourceType = textOf(row?['source_type']).isNotEmpty
        ...
        : (textOf(row?['source_customer_id']).isNotEmpty
            ? '成交客戶'
            : '自行新增');
    const sourceOptions = [
      '自行新增','成交客戶','情境','朋友','共電','其他',
    ];
    if(!sourceOptions.contains(sourceType)){
      sourceType='其他';
    }
    ...
    final values={
      ...
      'referred_by_name':textOf(row?['source_customer_id']).isNotEmpty &&
          ...
          ? textOf(row?['referred_by_name'])
          : sourceType,
      'notes':blank(notes.text),'status':row?['status']?.toString()??'增員中',
    };
    if(row==null) await repo.insert('recruitments',values);
    else await repo.update('recruitments',row['id'].toString(),values);
    await load();
  }
  ...
  @override Widget build(BuildContext context){
    super.build(context);
    final k=search.text.trim().toLowerCase();
    final filtered=rows.where((r)=>k.isEmpty||
      [r['name'],r['phone'],r['occupation'],r['company'],r['referred_by_name']]
        .whereType<Object>().join(' ').toLowerCase().contains(k)).toList()
      ..sort((a,b){
        final la=followUpLightFor(lastProgressFor(a));
        final lb=followUpLightFor(lastProgressFor(b));
        final byLight=followUpPriority(la).compareTo(followUpPriority(lb));
    ...
    return Scaffold(
      ...
      floatingActionButton:FloatingActionButton.extended(
        onPressed:()=>edit(),icon:const Icon(Icons.group_add_outlined),label:const Text('新增增員')),
      body:Column(children:[
        Container(
          padding:const EdgeInsets.fromLTRB(20,18,20,20),
          decoration:const BoxDecoration(
        ...
        Expanded(child:filtered.isEmpty?const Center(child:Text('目前沒有增員名單')):
          RefreshIndicator(onRefresh:load,child:ListView.separated(
            padding:const EdgeInsets.fromLTRB(16,18,16,100),itemCount:filtered.length,
              ...
              return Card(child:ListTile(
                onTap:() async{
                  await Navigator.push(context,MaterialPageRoute(
                    builder:(_)=>RecruitmentDetailPage(recruitment:row)));
                  await load();
                },
                title:Row(children:[
                  followUpDot(followUpLightFor(lastProgressFor(row)),size:10),
                  const SizedBox(width:7),
                  Expanded(child:Text(
                    nameWithRocBirthday(row['name'], row['birthday']),
                    style:const TextStyle(fontWeight:FontWeight.w900),
                ...
                subtitle:Text([
                  textOf(row['phone']),
                  '來源：${textOf(row['referred_by_name']).isNotEmpty
                      ? textOf(row['referred_by_name'])
                      : (textOf(row['source_type']).isNotEmpty
                          ? textOf(row['source_type'])
                          : '自行新增')}',
                ].where((e)=>e.isNotEmpty).join('\n')),
                trailing:PopupMenuButton<String>(
                  onSelected:(v) async{
                    if(v=='edit')await edit(row);
                    else await confirmDelete(context,() async{
                      await repo.remove('recruitments',row['id'].toString());await load();
                    });
                  },
                  itemBuilder:(_)=>const[
                    PopupMenuItem(value:'edit',child:Text('編輯')),
...
class RecruitmentDetailPage extends StatefulWidget {
  ...
  @override State<RecruitmentDetailPage> createState()=>_RecruitmentDetailPageState();
}
...
class _RecruitmentDetailPageState extends State<RecruitmentDetailPage>{
  ...
  Future<void> addLog() async{
    ...
    }catch(e){if(mounted)snack(context,'增員紀錄儲存失敗：$e');}
  }

  Future<void> editLog(Map<String,dynamic> row) async{
    final when=TextEditingController(text:formatDateOnly(row['recruited_on']));
    final status=TextEditingController(text:textOf(row['status_note']));
    final content=TextEditingController(text:textOf(row['content']));
    final saved=await showModalBottomSheet<bool>(
      context:context,isScrollControlled:true,useSafeArea:true,
  ...
  Future<void> copyToCustomer() async{
    ...
    final ok=await showDialog<bool>(context:context,builder:(context)=>AlertDialog(
      ...
      content:Text('將「${textOf(r['name'])}」複製到成交客戶？\n增員資料與增員紀錄都會保留，不會移除。'),
      actions:[
        TextButton(onPressed:()=>Navigator.pop(context,false),child:const Text('取消')),
        FilledButton(onPressed:()=>Navigator.pop(context,true),child:const Text('複製到客戶')),
      ]));
    if(ok!=true)return;
    try{
      final c=await repo.insertReturning('customers',{
        'name':textOf(r['name']),'phone':r['phone'],'line_id':r['line_id'],
        'birthday':r['birthday'],'occupation':r['occupation'],'company':r['company'],
        'family_status':r['family_status'],'notes':r['notes'],
    ...
    }catch(e){if(mounted)snack(context,'複製到客戶失敗：$e');}
  }
  ...
  @override Widget build(BuildContext context){
    ...
    return Scaffold(
      ...
      floatingActionButton:FloatingActionButton.extended(
        onPressed:addLog,icon:const Icon(Icons.add_comment_outlined),label:const Text('新增增員紀錄')),
      body:ListView(padding:const EdgeInsets.fromLTRB(16,8,16,110),children:[
        Card(child:Padding(padding:const EdgeInsets.all(18),child:Column(children:[
          infoRow('手機',textOf(r['phone'])),infoRow('LINE',textOf(r['line_id'])),
          infoRow('生日',formatRocBirthday(r['birthday'])),infoRow('職業',textOf(r['occupation'])),
          infoRow('公司',textOf(r['company'])),infoRow('家庭',textOf(r['family_status'])),
          infoRow(
            '來源',
        ...
        FilledButton.icon(onPressed:copyToCustomer,
          icon:const Icon(Icons.person_add_alt_1),label:const Text('增員轉為成交客戶（複製）')),
        const SizedBox(height:14),
        TextField(controller:search,onChanged:(_)=>setState((){}),
          decoration:const InputDecoration(labelText:'搜尋增員紀錄',prefixIcon:Icon(Icons.search),border:OutlineInputBorder())),
        sectionTitle('增員歷程 ${logs.length} 筆'),
        if(filtered.isEmpty)const Card(child:Padding(padding:EdgeInsets.all(18),child:Text('尚無符合的增員紀錄'))),
        ...filtered.map((e)=>Card(child:ListTile(
          onTap:()=>editLog(e),leading:const Icon(Icons.history),
          title:Text(textOf(e['content'])),
          subtitle:Text([formatDateOnly(e['recruited_on']),textOf(e['status_note'])]
            .where((x)=>x.isNotEmpty).join('\n')),
          trailing:PopupMenuButton<String>(
            onSelected:(v) async{
              if(v=='edit')await editLog(e);
              else await confirmDelete(context,() async{
                await repo.remove('recruitment_logs',e['id'].toString());await load();
              });
            },
            itemBuilder:(_)=>const[
              PopupMenuItem(value:'edit',child:Text('修改')),
...
class _PoliciesPageState extends State<PoliciesPage>
  ...
  @override void initState(){super.initState();load();}
  Future<void> load() async{
    final c=await repo.list('customers');final p=await repo.policies();
    if(mounted)setState((){customers=c;policies=p;});
  }
  @override Widget build(BuildContext context){
    super.build(context);final k=search.text.trim().toLowerCase();
    final filtered=customers.where((c)=>k.isEmpty||textOf(c['name']).toLowerCase().contains(k)).toList();
    return Scaffold(appBar:AppBar(title:const Text('保單管理')),body:Column(children:[
      Container(padding:const EdgeInsets.fromLTRB(20,18,20,20),
        decoration:const BoxDecoration(color:navy,borderRadius:BorderRadius.vertical(bottom:Radius.circular(34))),
        child:TextField(controller:search,onChanged:(_)=>setState((){}),
          decoration:InputDecoration(hintText:'搜尋客戶姓名',prefixIcon:const Icon(Icons.search),filled:true,fillColor:Colors.white,
            border:OutlineInputBorder(borderRadius:BorderRadius.circular(18),borderSide:BorderSide.none)))),
      Expanded(child:filtered.isEmpty?const Center(child:Text('尚無成交客戶')):
        RefreshIndicator(onRefresh:load,child:ListView.separated(
          padding:const EdgeInsets.fromLTRB(16,18,16,100),itemCount:filtered.length,
          separatorBuilder:(_,__)=>const SizedBox(height:10),itemBuilder:(context,index){
            final c=filtered[index];final id=c['id'].toString();
            final own=policies.where((e)=>e['customer_id']?.toString()==id).length;
            return Card(child:ListTile(
              onTap:() async{await Navigator.push(context,MaterialPageRoute(builder:(_)=>CustomerPoliciesPage(customer:c)));await load();},
              leading:const CircleAvatar(child:Icon(Icons.folder_copy_outlined)),
              title:Text(textOf(c['name']),style:const TextStyle(fontWeight:FontWeight.w900)),
              subtitle:Text('共 $own 張保單'),trailing:const Icon(Icons.chevron_right)));
          }))),
    ]));
  }
}
...
class CustomerPoliciesPage extends StatefulWidget {
  ...
  final Map<String,dynamic> customer;
  @override State<CustomerPoliciesPage> createState()=>_CustomerPoliciesPageState();
}

class _CustomerPoliciesPageState extends State<CustomerPoliciesPage> {
  final repo=Repo();List<Map<String,dynamic>> rows=[];
  static const insurers=<String>[
    '臺銀人壽','台灣人壽','保誠人壽','國泰人壽','凱基人壽','南山人壽','新光人壽','富邦人壽',
    '三商美邦人壽','遠雄人壽','宏泰人壽','安聯人壽','中華郵政','全球人壽','元大人壽','第一金人壽',
    '合作金庫人壽','安達國際人壽','友邦人壽','法國巴黎人壽','臺灣產物保險','兆豐產物保險',
    '富邦產物保險','和泰產物保險','泰安產物保險','明台產物保險','南山產物保險','第一產物保險',
    '旺旺友聯產物保險','新光產物保險','華南產物保險','國泰產物保險','新安東京海上產物保險',
    '中國信託產物保險','美國國際產物保險','美商安達產物保險','法國巴黎產物保險'
  ];
  String get customerId=>widget.customer['id'].toString();
  @override void initState(){super.initState();load();}
  Future<void> load() async{final all=await repo.policies();if(mounted)setState(()=>rows=all.where((e)=>e['customer_id']?.toString()==customerId).toList());}
  String premiumLabel(String f)=>f=='半年繳'?'半年繳保費':f=='季繳'?'季繳保費':f=='月繳'?'月繳保費':'年繳保費';
  double annualized(double a,String f)=>f=='半年繳'?a*2:f=='季繳'?a*4:f=='月繳'?a*12:a;

  Future<void> edit([Map<String,dynamic>? row]) async{
    final product=TextEditingController(text:textOf(row?['product_name']));
    String? insurer=textOf(row?['insurer']).isEmpty?null:textOf(row?['insurer']);
    String frequency=textOf(row?['payment_frequency']).isNotEmpty?textOf(row?['payment_frequency']):
      (textOf(row?['payment_method']).isNotEmpty?textOf(row?['payment_method']):'年繳');
    final premium=TextEditingController(text:textOf(row?['premium_amount']).isNotEmpty?textOf(row?['premium_amount']):textOf(row?['annual_premium']));
    final coverage=TextEditingController(text:textOf(row?['coverage_amount']));
    final effective=TextEditingController(text:textOf(row?['effective_date']));
    ...
    final existingRiders=row==null?<Map<String,dynamic>>[]:await repo.policyRiders(row['id'].toString());
    ...
    for(final r in existingRiders){
      riderForms.add({
        ...
        'frequency':textOf(r['payment_frequency']).isEmpty?'年繳':textOf(r['payment_frequency']),
        ...
        child:SingleChildScrollView(child:Column(children:[
          if(row==null)
            addModalHeader(
              ...
              ()=>[
                product.text,premium.text,coverage.text,effective.text,
                maturity.text,notes.text
              ].any((e)=>e.trim().isNotEmpty)||riderForms.isNotEmpty,
            )
          else
            Text('編輯保單｜${textOf(widget.customer['name'])}',
              style:const TextStyle(fontSize:22,fontWeight:FontWeight.w900)),
          const SizedBox(height:18),field(product,'主約 / 商品名稱 *'),
          DropdownButtonFormField<String>(initialValue:insurer,hint:const Text('請選擇保險公司'),isExpanded:true,
            decoration:const InputDecoration(labelText:'保險公司',border:OutlineInputBorder()),
            items:insurers.map((e)=>DropdownMenuItem(value:e,child:Text(e))).toList(),
            onChanged:(v)=>setSheet(()=>insurer=v)),
          const SizedBox(height:12),
          DropdownButtonFormField<String>(initialValue:frequency,
            decoration:const InputDecoration(labelText:'主約繳費方式',border:OutlineInputBorder()),
            items:const ['年繳','半年繳','季繳','月繳'].map((e)=>DropdownMenuItem(value:e,child:Text(e))).toList(),
            onChanged:(v){if(v!=null)setSheet(()=>frequency=v);}),
          const SizedBox(height:12),field(premium,premiumLabel(frequency),keyboardType:TextInputType.number),
          field(coverage,'主約保額',keyboardType:TextInputType.number),field(effective,'生效日（YYYY-MM-DD）'),
          ...
          Row(children:[
            const Expanded(child:Text('附約',style:TextStyle(fontSize:20,fontWeight:FontWeight.w900))),
            FilledButton.icon(
              ...
              icon:const Icon(Icons.add),label:const Text('新增附約')),
          ]),
          const SizedBox(height:8),
          if(riderForms.isEmpty)const Align(alignment:Alignment.centerLeft,child:Text('目前沒有附約')),
          ...List.generate(riderForms.length,(i){
            ...
            return Card(
              ...
              child:Padding(padding:const EdgeInsets.all(14),child:Column(children:[
                Row(children:[
                  Expanded(child:Text('附約 ${i+1}',style:const TextStyle(fontWeight:FontWeight.w900))),
                  IconButton(onPressed:()=>setSheet(()=>riderForms.removeAt(i)),icon:const Icon(Icons.delete_outline)),
                ]),
                field(r['name'] as TextEditingController,'附約名稱 *'),
                field(r['coverage'] as TextEditingController,'附約保額',keyboardType:TextInputType.number),
                DropdownButtonFormField<String>(initialValue:rf,
                  decoration:const InputDecoration(labelText:'附約繳費方式',border:OutlineInputBorder()),
                  items:const ['年繳','半年繳','季繳','月繳'].map((e)=>DropdownMenuItem(value:e,child:Text(e))).toList(),
                  onChanged:(v){if(v!=null)setSheet(()=>r['frequency']=v);}),
                const SizedBox(height:12),
                field(r['premium'] as TextEditingController,premiumLabel(r['frequency'] as String),keyboardType:TextInputType.number),
                field(r['paymentTerm'] as TextEditingController,'附約繳費年期 / 說明'),
                field(r['maturity'] as TextEditingController,'附約滿期日（YYYY-MM-DD）'),
                field(r['notes'] as TextEditingController,'附約備註',maxLines:2),
              ])),
            );
          }),
          const SizedBox(height:8),
          FilledButton(onPressed:()=>Navigator.pop(context,true),child:const Text('儲存保單')),
        ])))));

    if(saved!=true||product.text.trim().isEmpty||insurer==null)return;
    final amount=double.tryParse(premium.text)??0;
    final values={
      'customer_id':customerId,'product_name':product.text.trim(),'insurer':insurer,
      'payment_frequency':frequency,'payment_method':frequency,'premium_amount':amount,
      'annual_premium':annualized(amount,frequency),'coverage_amount':double.tryParse(coverage.text)??0,
      'effective_date':blank(effective.text),'maturity_date':blank(maturity.text),
      'riders_continue_after_maturity':ridersContinue,'notes':blank(notes.text)
    };
    String policyId;
    if(row==null){
      final created=await repo.insertReturning('policies',values);policyId=created['id'].toString();
    }else{
      policyId=row['id'].toString();await repo.update('policies',policyId,values);
    }
    ...
    for(final r in riderForms){
      ...
      final rf=r['frequency'] as String;
      final ra=double.tryParse((r['premium'] as TextEditingController).text)??0;
      riders.add({
        'name':name,
        'coverage_amount':double.tryParse((r['coverage'] as TextEditingController).text)??0,
  ...
  @override Widget build(BuildContext context){
    ...
    return Scaffold(
      ...
      bottomNavigationBar: mainNavigationBar(context, 1),
      floatingActionButton:FloatingActionButton.extended(onPressed:()=>edit(),icon:const Icon(Icons.add),label:const Text('新增保單')),
      body:rows.isEmpty?const Center(child:Text('這位客戶尚無保單')):
      RefreshIndicator(onRefresh:load,child:ListView.separated(
        padding:const EdgeInsets.fromLTRB(16,16,16,100),itemCount:rows.length,
        separatorBuilder:(_,__)=>const SizedBox(height:10),itemBuilder:(context,index){
          final row=rows[index];
          final f=textOf(row['payment_frequency']).isNotEmpty?textOf(row['payment_frequency']):textOf(row['payment_method']);
          final a=double.tryParse(textOf(row['premium_amount']).isNotEmpty?textOf(row['premium_amount']):textOf(row['annual_premium']))??0;
          return FutureBuilder<List<Map<String,dynamic>>>(
            future:repo.policyRiders(row['id'].toString()),
            builder:(context,snap){
              final riderCount=snap.data?.length??0;
              return Card(child:ListTile(
                onTap:()=>edit(row),leading:const CircleAvatar(child:Icon(Icons.description_outlined)),
                title:Text(textOf(row['product_name']),style:const TextStyle(fontWeight:FontWeight.w900)),
                subtitle:Text([
                  textOf(row['insurer']),f,'${premiumLabel(f)} ${currency.format(a)} 元',
                  textOf(row['maturity_date']).isEmpty?'':'主約滿期 ${textOf(row['maturity_date'])}',
                  '附約 $riderCount 個${row['riders_continue_after_maturity']==true?'・主約滿期後仍需繳費':''}'
                ].where((e)=>e.isNotEmpty).join('\n')),
                trailing:PopupMenuButton<String>(onSelected:(v) async{
                  if(v=='edit')await edit(row);else await confirmDelete(context,() async{await repo.remove('policies',row['id'].toString());await load();});
                },itemBuilder:(_)=>const[
                  PopupMenuItem(value:'edit',child:Text('編輯主約 / 附約')),
                  PopupMenuItem(value:'delete',child:Text('刪除'))
                ])));
            });
        })));
...
class _ReferralTreePageState extends State<ReferralTreePage> {
  ...
  List<Map<String, dynamic>> referralsOf(String customerId) =>
      contacts
          .where((e) => e['customer_id']?.toString() == customerId)
          .toList();
...
String formatDateOnly(dynamic raw) {
  ...
  if (dt == null) return value.length >= 10 ? value.substring(0, 10).replaceAll('-', '/') : value;
